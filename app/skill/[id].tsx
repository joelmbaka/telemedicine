...
import { ActivityIndicator, ScrollView, StyleSheet, Text, View, TouchableOpacity, FlatList, Image } from 'react-native';
...
export default function SkillDetailPage() {
  const { id, doctorName, doctorAvatar } = useLocalSearchParams<{ id: string; doctorName?: string; doctorAvatar?: string }>();
  ...
  const fetchSkillDetails = async () => {
    try {
      ...
      // Fetch doctor details (prefer doctors -> profiles to avoid RLS issues on public page)
      ...
      // Fetch reviews (mock data for now - you'll need to create a reviews table)
      // For now, I'll create some sample reviews
            ...
            <View style={styles.statItem}>
              <View style={styles.ratingContainer}>
                ...
                <Text style={styles.ratingText}>
                  {skill.avg_rating?.toFixed(1) || '0.0'}
                </Text>
              </View>
            </View>
            <View style={styles.statItem}>
        ...
        {doctor && (
          <View style={styles.section}>
            <TouchableOpacity 
              ...
              onPress={() => router.push(`/appointments/new?doctorId=${doctor.id}&skillId=${skill.id}`)}
            >
